...
from src.prompts import COMPILER_SYSTEM_PROMPT, classification_prompt, retry_classification_prompt
        ...
        if line_type == "map":
            ...
            if not map_stack:
                ...
                retry_response = conversation.chat(
                    retry_classification_prompt(line), response_schema=CLASSIFICATION_SCHEMA
                )
                ...
                reduce_stmt = _parse_reduce_line(line)
                
                # Finalize any open maps and add them to statements
                while map_stack:
                    completed_map = map_stack.pop()
                    statements.append(completed_map)
                
                # Add the reduce statement
                statements.append(reduce_stmt)
...
def _parse_map_line(line: str, tools: list[str]) -> Map:
    ...
    dimension_command = Command(prompt=line, tools=tools)
    return Map(dimension=dimension_command, body=Program([]))
